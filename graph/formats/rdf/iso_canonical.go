...
func (t table) clone() table {
	...
	new.blanks = t.blanks
	if t.isBlank != nil {
		// This is our first clone operation, so make
		// a slice copy of the isBlank set.
		new.blanks = make([]string, len(t.isBlank))
		i := 0
...
func (t table) set(term string, hash []byte) {
	...
	if t.isBlank != nil && strings.HasPrefix(term, "_:") {
...
// the given hash table. newHashBag can only be take a table parameter
// that was returned by newTable.
...
// sum returns calculates the hash sum for the given term, updates the
// hash bag state and returns the hash.
...
// hashTuble returns the h hash of the concatenation of t.
