...
// receiver. Changes to the DL, D, DU slices in lapack64.Tridiagonal will be
// reflected in the original matrix, changes to the N field will not.
...
func (a *Tridiag) T() Matrix {
	// TODO(vladimir-ch): should we return the receiver with DL,DU swapped?
	return Transpose{a}
}
...
func (a *Tridiag) Reset() {
	a.mat.N = 0
	a.mat.DL = a.mat.DL[:0:0]
	a.mat.D = a.mat.D[:0:0]
	a.mat.DU = a.mat.DU[:0:0]
}
...
func (a *Tridiag) SolveTo(dst *Dense, trans bool, b Matrix) error {
	...
	var ok bool
	if trans {
		ok = lapack64.Gtsv(blas.Trans, a.mat, dst.mat)
	} else {
		ok = lapack64.Gtsv(blas.NoTrans, a.mat, dst.mat)
	}
	if !ok {
...
// an n×n tridiagonal matrix represented by the receiver and b is a given
// n-vector. If A is non-singular, the result will be stored into dst and nil
// will be returned. If A is singular, the contents of dst will be undefined and
...
func (a *Tridiag) SolveVecTo(dst *VecDense, trans bool, b Vector) error {
	...
	var ok bool
	if trans {
		ok = lapack64.Gtsv(blas.Trans, a.mat, dst.asGeneral())
	} else {
		ok = lapack64.Gtsv(blas.NoTrans, a.mat, dst.asGeneral())
	}
	if !ok {
		return Condition(math.Inf(1))
	}
